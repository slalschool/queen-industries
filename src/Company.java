...
public class Company {
    ...
    private Director getDirector(String department) {
        for (Employee dtr : staff) {
            if (dtr.getDepartment().equals(department)) {
                return (Director) dtr;
            }
        }
        return null;
    ...
    private Employee getEmployee(String name) {
        for (Employee dtr : staff) {
            Director dtr1 = (Director) dtr;
            for (Employee mng : dtr1.getManagers()) {
                if (mng.getName().equals(name)) {
                    return mng;
                }
                Manager mng1 = (Manager) mng;
                for (Employee emp : mng1.getEmployees()) {
    ...
    private void printEmployeeDetails(Director director) {
        ...
        System.out.println("Title        : " + director.getTitleString());
    ...
    private void printEmployeeDetails(Manager manager) {
        ...
        System.out.println("Title        : " + manager.getTitleString());
    ...
    private void printEmployeeDetails(Employee employee) {
        ...
        System.out.println("Title        : " + employee.getTitleString());
    ...
    private void printOrganizationChart() {
        ...
        if (staff.size() > 0) {
            ...
            sortStaff();
            for (Employee dtr : staff) {
                System.out.println(" - " + dtr.getName() + ", " + dtr.getTitleString());
                for (Employee mng : ((Director) dtr).getManagers()) {
                    if (mng.getTier() <= 1) {
                        System.out.println("    - " + mng.getName() + ", " + mng.getTitleString());
                    } else {
                        System.out.println("    - " + mng.getName() + ", " + mng.getTitleString());
                        for (Employee emp : ((Manager) mng).getEmployees()) {
                            System.out.println("       - " + emp.getName() + ", " + emp.getTitleString());
                        }
                    }
                }
            }
    ...
    public void sortStaff() {
        ArrayList<Employee> tempStaff = new ArrayList<>();
        int size = staff.size();
        Employee first;
        for (int i = 0; i < size; i++) {
            first = staff.get(0);
            for (Employee dtr : staff) {
                if (dtr.getDepartment().compareTo(first.getDepartment()) < 0) first = dtr;
            }
